-- ============================================================================
-- TuCanchera — SETUP COMPLETO para un proyecto Supabase NUEVO
--
-- Generado el 2026-08-06 para el proyecto "TuCanchera-dev" (lgwqqgwokudqygeptuqc)
--
-- Combina, EN ORDEN:
--   1) supabase/schema.sql                    (estado al 2026-05-21)
--   2) migración 20260522000001               (get_disponibilidad_semana)
--
-- La migración 2 NO estaba incluida en schema.sql y el front la usa en
-- src/services/complejoService.ts:72 para la grilla semanal de turnos.
-- Sin ella, la pantalla de reservar rompe.
--
-- ⚠️  ANTES DE EJECUTAR: confirmá arriba a la izquierda del dashboard que el
--     proyecto seleccionado NO es ztwtxrsanjehzpdbitxg (ese es PRODUCCIÓN).
--
-- Requiere las extensiones pg_cron y pgcrypto habilitadas desde
-- Database → Extensions.
-- ============================================================================
...
-- ############################################################################
-- ### PARTE 2: migración 20260522000001_get_disponibilidad_semana
-- ############################################################################
...
-- ============================================================================
-- VERIFICACIÓN — corré esto después y comprobá los números esperados
-- ============================================================================
-- select count(*) from information_schema.tables   where table_schema='public';    -- 10
-- select count(*) from information_schema.routines where routine_schema='public';  --  9
-- select id from storage.buckets;                          -- logos, fotos-complejos
